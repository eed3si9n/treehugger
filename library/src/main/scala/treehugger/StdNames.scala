...
trait StdNames extends NameManglers { self: Forest =>
  ...
  trait TypeNames extends CommonNames {
    ...
    final val ClassfileAnnotationATTR: NameType =
      "RuntimeInvisibleAnnotations" // RetentionPolicy.CLASS. Currently not used (Apr 2009).
    ...
    final val RuntimeParamAnnotationATTR: NameType =
  ...
  trait TermNames extends Keywords with CommonNames {
    ...
    val clone_ : NameType          =
      if (forMSIL) "MemberwiseClone"
      else "clone" // sn.OClone causes checkinit failure
  ...
  object tpnme
      extends TypeNames /*with LibraryTypeNames*/
      with TypeNameMangling {
  ...
  object nme extends TermNames /*with LibraryTermNames*/ with TermNameMangling {
    ...
    /**
     * Translate a String into a list of simple TypeNames and TermNames. In all
     * segments before the last, type/term is determined by whether the
     * following separator char is '.' or '#'. In the last segment, the
     * argument "assumeTerm" determines it. Examples:
     *
     * package foo { object Lorax { object Wog ; class Wog } class Lorax {
     * object Zax ; class Zax } }
     *
     ...
     *
     * Note that in actual scala syntax you cannot refer to object Zax without
     * an instance of Lorax, so Lorax#Zax could only mean the type. One might
     * think that Lorax#Zax.type would work, but this is not accepted by the
     * parser. For the purposes of referencing that object, the syntax is
     * allowed.
     */
    ...
    val BITMAP_PRIVATE: NameType =
      BITMAP_PREFIX + "priv$" // initialization bitmap for private lazy vals
    ...
    def moduleVarName(name: TermName): TermName = newTermName(
  ...
  abstract class SymbolNames {
    protected implicit def stringToTypeName(s: String): TypeName = newTypeName(
      s
    )
  ...
  private class MSILNames extends SymbolNames {
    ...
    final val GetCause: TermName =
      "InnerException" /* System.Reflection.TargetInvocationException.InnerException */
