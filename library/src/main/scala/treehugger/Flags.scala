...
class ModifierFlags {
  ...
  final val ABSTRACT =
    0x00000008 // abstract class, or used in conjunction with abstract override.
  // Note difference to DEFERRED!
  final val DEFERRED =
    0x00000010 // was `abstract' for members | trait is virtual
  final val INTERFACE =
    0x00000080 // symbol is an interface (i.e. a trait which defines only abstract methods)
  final val MUTABLE = 0x00001000 // symbol is a mutable variable.
  final val PARAM =
    0x00002000 // symbol is a (value or type) parameter to a method
  ...
  final val CONTRAVARIANT =
    0x00020000 // symbol is a contravariant type variable
  final val ABSOVERRIDE = 0x00040000 // combination of abstract & override
  final val LOCAL =
    0x00080000 // symbol is local to current class (i.e. private[this] or protected[this]
  ...
  final val CASEACCESSOR =
    0x01000000 // symbol is a case parameter (or its accessor)
  ...
  final val PARAMACCESSOR =
    0x20000000 // for field definitions generated for primary constructor
  ...
  final val LAZY =
    0x80000000L // symbol is a lazy val. can't have MUTABLE unless transformed by typer
  final val PRESUPER = 0x2000000000L // value is evaluated before super call
  final val DEFAULTINIT =
    0x20000000000L // symbol is initialized to the default value: used by -Xcheckinit

  // treehugger custom
  final val HEADER =
    0x1000000000000L // symbol is a non-block header (i.e. package scala.collection)
...
class Flags extends ModifierFlags {
  ...
  final val MODULE =
    0x00000100 // symbol is module or class implementing a module
  final val PACKAGE = 0x00004000 // symbol is a java package

  final val CAPTURED =
    0x00010000 // variable is accessed from nested function.  Set by LambdaLift.
  final val LABEL = 0x00020000 // method symbol is a label. Set by TailCall
  final val INCONSTRUCTOR =
    0x00020000 // class symbol is defined in this/superclass constructor.
  ...
  final val ACCESSOR =
    0x08000000 // a value or variable accessor (getter or setter)

  final val SUPERACCESSOR = 0x10000000 // a super accessor
  final val MODULEVAR =
    0x40000000 // for variables: is the variable caching a module value
  ...
  final val EXISTENTIAL =
    0x800000000L // type is an existential parameter or skolem
  final val EXPANDEDNAME =
    0x1000000000L // name has been expanded with class suffix
  final val IMPLCLASS = 0x2000000000L // symbol is an implementation class
  final val TRANS_FLAG =
    0x4000000000L // transient flag guaranteed to be reset after each phase.

  final val LOCKED =
    0x8000000000L // temporary flag to catch cyclic dependencies
  final val SPECIALIZED =
    0x10000000000L // symbol is a generated specialized member
  ...
  final val TRIEDCOOKING =
    0x100000000000L // ``Cooking'' has been tried on this symbol
  ...
  final val InitialFlags =
    0x0001ffffffffffffL // flags that are enabled from phase 1.
  final val LateFlags =
    0x00fe000000000000L // flags that override flags in 0x1FC.
  ...
  /** These flags can be set when class or module symbol is first created. They
    * are the only flags to survive a call to resetFlags().
    */
  final val TopLevelCreationFlags: Long =
    MODULE | PACKAGE | FINAL | JAVA

  /** These modifiers can be set explicitly in source programs. This is used
    * only as the basis for the default flag mask (which ones to display when
  ...
  /** When a symbol for a field is created, only these flags survive from
    * Modifiers. Others which may be applied at creation time are: PRIVATE,
    * LOCAL.
    */
  final val FieldFlags: Long =
    MUTABLE | CASEACCESSOR | PARAMACCESSOR | STATIC | FINAL | PRESUPER | LAZY

  /** Masks for getters and setters, where the flags are derived from those on
    * the field's modifiers. Both getters and setters get the ACCESSOR flag.
  ...
  /** When a symbol for a default getter is created, it inherits these flags
    * from the method with the default. Other flags applied at creation time are
    * SYNTHETIC, DEFAULTPARAM, and possibly OVERRIDE.
    */
  ...
  /** These appear to be flags which should be transferred from owner symbol to
    * a newly created constructor symbol.
    */
  final val ConstrFlags: Long = JAVA

  /** Module flags inherited by their module-class */
  final val ModuleToClassFlags: Long =
    AccessFlags | MODULE | PACKAGE | CASE | SYNTHETIC | JAVA | FINAL
  ...
  def flagsToString(flags: Long): String =
    pickledListOrder map (mask =>
      flagToString(flags & mask)
  ...
  def modifiersOfFlags(flags: Long): List[Modifier.Value] =
    pickledListOrder map (mask => modifierOfFlag(flags & mask)) flatMap { mod =>
      mod
    }
}
