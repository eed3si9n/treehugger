...
trait NameManglers { self: Forest =>
  ...
  trait TermNameMangling extends NameManglingCommon {
    ...
    def getterToLocal(name: TermName): TermName =
      name.toTermName append LOCAL_SUFFIX_STRING
    def getterToSetter(name: TermName): TermName =
      name.toTermName append SETTER_SUFFIX
    def localToGetter(name: TermName): TermName =
      (name stripEnd LOCAL_SUFFIX_STRING).toTermName

    def setterToGetter(name: TermName): TermName = {
      val p = name.pos(TRAIT_SETTER_SEPARATOR_STRING)
      if (p < name.length)
        setterToGetter(
          name.subName(p + TRAIT_SETTER_SEPARATOR_STRING.length, name.length)
        )
      else
    ...
    def implClassName(name: Name): TypeName =
      (name append IMPL_CLASS_SUFFIX).toTypeName
    def interfaceName(implname: Name): TypeName =
      (implname stripEnd IMPL_CLASS_SUFFIX).toTypeName
