...
class DSL_2ClassSpec extends DSLSpec {
  def is = s2"""
...
`CLASSDEF(sym|"C") := BLOCK(DEF(sym|"get", typ) := rhs, ...)`.                $member1
  ...
  def class3 = {
    val tree: Tree = CLASSDEF("C") withParams (
      PARAM("x", IntClass),
      VAL("y", StringClass),
      VAR("z", TYPE_LIST(StringClass))
    )
  ...
  def class4 =
    ((CLASSDEF("Queue") withTypeParams (TYPEVAR("T"))
      withParams (
        VAL("leading", TYPE_LIST("T")),
        VAL("trailing", TYPE_LIST("T"))
      ): Tree) must print_as("class Queue[T](val leading: List[T], val trailing: List[T])")) and
      ((CLASSDEF("C") withTypeParams (TYPEVAR("A") :: TYPEVAR("B") :: Nil)).tree must print_as(
        "class C[A, B]"
      ))

  def class5 =
    (CLASSDEF("C") withParents (sym.Addressable): Tree) must print_as("class C extends Addressable")
  ...
  def caseclass1 =
    ((CASECLASSDEF("C"): Tree) must print_as("case class C()")) and
      ((CASECLASSDEF("C") withParams (VAL(
        "x",
        IntClass
      ) withFlags (Flags.OVERRIDE)) withParents (sym.Addressable APPLY (REF("x"))) := BLOCK(
        DEF("y") := LIT(0)
      ))
        must print_as(
