...
class TreePrinterSpec extends DSLSpec {
  def is = sequential ^
  ...
  def e2 = {
    val tree = PROC("hello") := BLOCK(Predef_println APPLY LIT("Hello, world!"))
    val s    = treeToString(tree); println(s)

    s.linesIterator.toList must contain(
      allOf(
        """def hello {""",
        """  println("Hello, world!")""",
        """}"""
  ...
  def e4 = {
    ...
    out.linesIterator.toList must contain(
      allOf(
    ...
    val trees =
      (CLASSDEF(IntQueue) withFlags (Flags.ABSTRACT) := BLOCK(
        ...
        (TRAITDEF(Doubling) withParents (IntQueue) := BLOCK(
          PROC("put") withFlags (Flags.ABSTRACT, Flags.OVERRIDE) withParams (PARAM(
            "x",
            IntClass
          )) := BLOCK(
    ...
    val tree =
      (OBJECTDEF(PredefModule) := BLOCK(
        (CLASSDEF(ArrowAssocClass) withTypeParams (TYPEVAR(A)) withParams (PARAM("x", A)) := BLOCK(
          DEF(arrow.name, TYPE_TUPLE(A, B)) withTypeParams (TYPEVAR(B)) withParams (PARAM(
            "y",
            B
          )) :=
            TUPLE(REF("x"), REF("y"))
  ...
  def e8 = {
    ...
    val tree: Tree =
      (CASECLASSDEF(Address)
        ...
        DEF("stringOnly", Address) withParams (PARAM(
          "ev",
          TYPE_=:=(T, StringClass)
        ...
        DEF("star") withParams (PARAM("n", TYPE_*(IntClass))) :=
          Address APPLYTYPE (StringClass) APPLY SOME(LIT("foo"))
            .MAP(WILDCARD INFIX (StringAdd_+) APPLY LIT("x"))
      ))
  ...
  def e10 = {
    ...
    val tree: Tree =
      (DEF(sym.mkPointed)
        ...
        NEW(
          ANONDEF(
            sym.Pointed TYPE_OF (TYPE_STRUCT(
            ...
            DEF("point")
              ...
              (sym.Const APPLYTYPE (
                "M",
                "A"
              ) APPLY (Predef_implicitly APPLYTYPE (sym.Monoid TYPE_OF "M") DOT "z"))
          )
        ))
