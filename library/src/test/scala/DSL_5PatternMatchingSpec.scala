...
class DSL_5PatternMatchingSpec extends DSLSpec {
  def is = s2"""
...
`tree MATCH (CASE(pattern1, IF(guard)) ==> tree1, ...)`.                      $pattern15
  ...
  def pattern15 =
    REF("x") MATCH (
      CASE(ID("x"), IF(REF("x") INT_< LIT(10))) ==> TRUE,
      CASE(WILDCARD) ==> FALSE
    ) must print_as(
      """x match {""",
      """  case x if x < 10 => true""",
  ...
  def patternvalue1 =
    ((VAL(
      REF("Address") UNAPPLY
        (ID("name"), ID("street"), ID("city"))
    ) := REF("x")) must print_as("val Address(name, street, city) = x")) and
