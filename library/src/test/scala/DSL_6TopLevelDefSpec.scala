...
class DSL_6TopLevelDefSpec extends DSLSpec {
  def is = s2"""
  ...
  def unit1 =
    (PACKAGEHEADER("p") := BLOCK(
      OBJECTDEF("M")
    )) must print_as("package p", "", "object M")

  def unit2 =
    (BLOCK(
      OBJECTDEF("M")
    ) inPackage ("p")) must print_as("package p", "", "object M")
  ...
  def packageobj1 =
    (PACKAGEOBJECTDEF("p") := BLOCK(
      OBJECTDEF("M")
    )) must print_as("package object p {", "  object M", "}")
}
